...
def coco_only_people(path='../coco/labels/train2017/'):  # from utils.utils import *; coco_only_people()
    # Find faceMask with only people
    files = sorted(glob.glob('%s/*.*' % path))
    for i, file in enumerate(files):
        labels = np.loadtxt(file, dtype=np.float32).reshape(-1, 5)
        if all(labels[:, 0] == 0):
...
def crop_images_random(path='../faceMask/', scale=0.50):  # from utils.utils import *; crop_images_random()
    # crops faceMask into random squares up to scale fraction
    # WARNING: overwrites faceMask!
...
def coco_single_class_labels(path='../coco/labels/train2014/', label_class=43):
    ...
    os.makedirs('new/faceMask/')
    for file in tqdm(sorted(glob.glob('%s/*.*' % path))):
        with open(file, 'r') as f:
            labels = np.array([x.split() for x in f.read().splitlines()], dtype=np.float32)
        i = labels[:, 0] == label_class
        if any(i):
            img_file = file.replace('labels', 'faceMask').replace('txt', 'jpg')
            labels[:, 0] = 0  # reset class to 0
            with open('new/faceMask.txt', 'a') as f:  # add image to dataset list
                f.write(img_file + '\n')
            with open('new/labels/' + Path(file).name, 'a') as f:  # write label
                for l in labels[i]:
                    f.write('%g %.6f %.6f %.6f %.6f\n' % tuple(l))
            shutil.copyfile(src=img_file, dst='new/faceMask/' + Path(file).name.replace('txt', 'jpg'))  # copy faceMask
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    # red for nomask, green for mask
    if label.split(' ')[0] == 'face_mask':
        color = [25, 175, 25]
    if label.split(' ')[0] == 'face':
        color = [0, 0, 255]
...
def plot_images(images, targets, paths=None, fname='faceMask.jpg', names=None, max_size=640, max_subplots=16):
    tl = 3  # line thickness
    tf = max(tl - 1, 1)  # font thickness
    ...
    bs = min(bs, max_subplots)  # limit plot faceMask
    ...
    for i, img in enumerate(images):
        if i == max_subplots:  # if last batch has fewer faceMask than we expect
            break
