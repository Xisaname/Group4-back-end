...
def create_modules(module_defs, img_size, cfg):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            filters = int(float(mdef['filters']))
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), verbose=False):
        ...
        self.seen = np.array([0], dtype=np.int64)  # (int64) number of faceMask seen during training
    ...
    def forward(self, x, augment=False, verbose=False):
        ...
        else:  # Augment faceMask (inference and test only) https://github.com/ultralytics/yolov3/issues/931
    ...
    def forward_once(self, x, augment=False, verbose=False):
        ...
        # Augment faceMask (inference and test only)
...
def load_darknet_weights(self, weights, cutoff=-1):
    ...
    with open(weights, 'rb') as f:
        ...
        self.seen = np.fromfile(f, dtype=np.int64, count=1)  # (int64) number of faceMask seen during training
...
def save_weights(self, path='model.weights', cutoff=-1):
    ...
    with open(path, 'wb') as f:
        ...
        self.seen.tofile(f)  # (int64) number of faceMask seen during training
